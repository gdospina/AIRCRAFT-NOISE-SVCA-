...
"""Procesamiento Jornada Nocturno"""
cwd = os.path.abspath('') 
files = os.listdir(cwd) 
df = pd.DataFrame()
for file in files:
    if file.endswith('.xlsx'):
        df = df.append(pd.read_excel(file, sheet_name='NOCTURNO'), ignore_index=True) #Reconocimiento de archivos de excel
df.head()
df.info()
...
print(df_final)
'NMT' in df_final
#df.groupby('a').apply(lambda x: x['b'].sum()/x['c'].sum())
#df_final=df_final.set_index(['Hora'])#.set_index funciona para configurar el index por columna especifica

"""PROCESAMIENTO EVALUACION FASE II"""

siete=df_final.loc[df_final['Hora'] < 8]
siete['LEVEL_00-07 POT']= 10**(siete['LEVEL_H']/10)
l_siete=siete.groupby(["Date","NMT"],dropna=False)['LEVEL_00-07 POT'].sum().reset_index()
l_siete['LEVEL_00-07'] = 10*np.log10((1/7)*l_siete['LEVEL_00-07 POT'])
print(l_siete)
cinco=df_final.loc[df_final['Hora'] < 5]
cinco['LEVEL_00-05 POT']= 10**(cinco['LEVEL_H']/10)
l_cinco=cinco.groupby(["Date","NMT"],dropna=False)['LEVEL_00-05 POT'].sum().reset_index()
l_cinco['LEVEL_00-05'] = 10*np.log10((1/5)*l_cinco['LEVEL_00-05 POT'])
cinco_siete=df_final.loc[(df_final['Hora'] >= 5) & (df_final['Hora'] < 8)]
cinco_siete['LEVEL_05-07 POT']= 10**(cinco_siete['LEVEL_H']/10)
l_cinco_siete=cinco_siete.groupby(["Date","NMT"],dropna=False)['LEVEL_05-07 POT'].sum().reset_index()
l_cinco_siete['LEVEL_05-07'] = 10*np.log10((1/2)*l_cinco_siete['LEVEL_05-07 POT'])

LA=pd.DataFrame()
LA['LEVEL_00-05']=l_cinco['LEVEL_00-05']
LA['LEVEL_00-07']=l_siete['LEVEL_00-07']
LA['LEVEL_05-07']=l_cinco_siete['LEVEL_05-07']
LA


"""
df_final.loc[21]
df_final=df_final.drop('21')
df_final.head()
df_final.drop((df_final['Hora'] >0) & (df_final['Hora'] < 6).index(),inplace=True)
df_final.groupby("NMT").apply()"""


"""Calculo nivel diario Ln"""
df_final['LEVEL_H POT']= 10**(df_final['LEVEL_H']/10)
df_ln=df_final.groupby(["Date","NMT"])['LEVEL_H POT'].sum().reset_index()
df_ln['LEVEL_LD']= 10*np.log10((1/10)*df_ln['LEVEL_H POT'])
del df_ln['LEVEL_H POT']#Borra una columna especifica de un DataFrame
...
df_ln.columns
df_ln.index#Verifica cual es el index
df_ln=df_ln.set_index(['Date'])#Configrar el index por una variable especifica
'Date' in df_ln#Verifica si el valor se ecuentra en el DataFrame
...
niv_air_type_f1['LEVEL_Aircraft_type'] = 10*np.log10((1/(3600*10))*niv_air_type_f1['SEL OACI POT'])#Calcula el nivel equivalente diario para cada modelo de aeronave
...
niv_air_type_perday['LEVEL_Aircraft_type'] = 10*np.log10((1/(3600*10))*niv_air_type_perday['SEL OACI POT'])#Calcula el nivel equivalente diario para cada modelp

niv_air_type_permonth['LEVEL_Aircraft_type'] = 10*np.log10((1/(3600*10*31))*niv_air_type_permonth['SEL OACI POT'])#Calcula el nivel equivalente mensual para cada modelp
...
for c in index_list_all:
    ...
    B738_ARR = A319_DIR.get_group('ARR')
...
#%%Pruebas de codigo
"""F_1=df_ln.loc['F001']#Busca los valores asociados a un objeto especifico
print(F_1)
'Date' in df_ln
Tdf_ln=df_ln.T
'F001' in Tdf_ln#Verifica si el valor se ecuentra en el DataFrame

a=df_ln.loc[2]#Buscar valores de fila por indicador de Index

print("the shape is 2 rows, 3 columns: ", df_ln.shape)
print(type(df_ln))
...
F_1=df_ln.groupby('NMT')#Agrupa los datos en tipo NoneType
...
lim_est_0627_ln = pd.DataFrame({'F1' :  np.repeat((75), len(dates)),
                   'F2': np.repeat((55), len(dates)),
                   'F3': np.repeat((55), len(dates)),
                   'F4': np.repeat((50), len(dates)),
                   'F5': np.repeat((50), len(dates)),
                   'F7': np.repeat((60), len(dates)),
                   'F8': np.repeat((60), len(dates)),
                   'F10': np.repeat((55), len(dates)),
                   'F11': np.repeat((50), len(dates)),
                   'F13': np.repeat((75), len(dates)),
                   'F15': np.repeat((75), len(dates)),
                   'F17': np.repeat((50), len(dates)),
                   'F18': np.repeat((55), len(dates)),
                   'F19': np.repeat((75), len(dates)),
                   'F20': np.repeat((75), len(dates)),
                   'F21': np.repeat((60), len(dates)),
                   'F23': np.repeat((55), len(dates)),
                   'F24': np.repeat((55), len(dates)),
                   'F25': np.repeat((75), len(dates)),
                   'F27': np.repeat((75), len(dates)),
                   'F28': np.repeat((75), len(dates)),
                   'F29': np.repeat((50), len(dates)),
                   'F30': np.repeat((60), len(dates)),
                   'F32': np.repeat((75), len(dates)),
                   'F33': np.repeat((55), len(dates)),
                   'F34': np.repeat((55), len(dates))},index=dates)
...
graph_top_max.set_title('Niveles máximos de contribución acústica nocturnos - (Diciembre-2022)', fontsize=20, fontweight='bold')
graph_top_max.set_xlabel('LAeqMax, Night (dBA)', fontsize=17)
...
f32_LEN=f32_LEN.rename({'SEL': 'F30'}, axis=1)
...
Count_Events = pd.concat([f1_LEN, f2_LEN, f3_LEN, f4_LEN, f5_LEN, f7_LEN, f8_LEN, f10_LEN, f11_LEN, f13_LEN, f15_LEN,
                          ...
                          f29_LEN, f30_LEN, f32_LEN, f33_LEN, f34_LEN],axis=1)
...
nocturno=pd.concat([F1,F2,F3,F4,F5,F7,F8,F10,F11,F13,F15,F17,F18,F19,F20,F21,F23,F24,F25,F27,F28,F29,F30,F32,F33,F34],axis=1)
min_val = nocturno.min().min()
nocturno=nocturno.round(1)
...
comp_res0627=nocturno.subtract(lim_est_0627_ln)
...
bins = np.arange(min_val,nocturno.stack().max()+1,3.0)
freq_table=pd.concat([nocturno.groupby(pd.cut(nocturno.F1, bins=bins)).F1.count(), 
                      nocturno.groupby(pd.cut(nocturno.F2, bins=bins)).F2.count(), 
                      nocturno.groupby(pd.cut(nocturno.F3, bins=bins)).F3.count(), 
                      nocturno.groupby(pd.cut(nocturno.F4, bins=bins)).F4.count(),
                      nocturno.groupby(pd.cut(nocturno.F5, bins=bins)).F5.count(),
                      nocturno.groupby(pd.cut(nocturno.F7, bins=bins)).F7.count(),
                      nocturno.groupby(pd.cut(nocturno.F8, bins=bins)).F8.count(),
                      nocturno.groupby(pd.cut(nocturno.F10, bins=bins)).F10.count(),
                      nocturno.groupby(pd.cut(nocturno.F11, bins=bins)).F11.count(),
                      nocturno.groupby(pd.cut(nocturno.F13, bins=bins)).F13.count(),
                      nocturno.groupby(pd.cut(nocturno.F15, bins=bins)).F15.count(),
                      nocturno.groupby(pd.cut(nocturno.F17, bins=bins)).F17.count(),
                      nocturno.groupby(pd.cut(nocturno.F18, bins=bins)).F18.count(),
                      nocturno.groupby(pd.cut(nocturno.F19, bins=bins)).F19.count(),
                      nocturno.groupby(pd.cut(nocturno.F20, bins=bins)).F20.count(),
                      nocturno.groupby(pd.cut(nocturno.F21, bins=bins)).F21.count(),
                      nocturno.groupby(pd.cut(nocturno.F23, bins=bins)).F23.count(),
                      nocturno.groupby(pd.cut(nocturno.F24, bins=bins)).F24.count(),
                      nocturno.groupby(pd.cut(nocturno.F25, bins=bins)).F25.count(),
                      nocturno.groupby(pd.cut(nocturno.F27, bins=bins)).F27.count(),
                      nocturno.groupby(pd.cut(nocturno.F28, bins=bins)).F28.count(),
                      nocturno.groupby(pd.cut(nocturno.F29, bins=bins)).F29.count(),
                      nocturno.groupby(pd.cut(nocturno.F30, bins=bins)).F30.count(),
                      nocturno.groupby(pd.cut(nocturno.F32, bins=bins)).F32.count(),
                      nocturno.groupby(pd.cut(nocturno.F33, bins=bins)).F33.count(),
                      nocturno.groupby(pd.cut(nocturno.F34, bins=bins)).F34.count()],axis = 1)
...
ax_eng.set_title('Niveles nocturnos de ruido aeronáutico - Localidad de Engativá - (Diciembre-2022)', fontsize = 20, fontweight='bold')
ax_eng.set_xlabel('Jornadas Nocturnas',fontsize = 17)
ax_eng.set_ylabel('LAeq, Night (dBA)',fontsize = 17)
...
ax_fon.set_title('Niveles nocturnos de ruido aeronáutico - Localidad de Fontibón -  (Diciembre-2022)', fontsize = 20, fontweight='bold')
ax_fon.set_xlabel('Jornadas Nocturnas',fontsize = 17)
ax_fon.set_ylabel('LAeq, Night (dBA)',fontsize = 17)
...
ax_fun.set_title('Niveles nocturnos de ruido aeronáutico - Localidad de Funza -  (Diciembre-2022)', fontsize=20, fontweight='bold')
ax_fun.set_xlabel('Jornadas Nocturnas', fontsize=17)
ax_fun.set_ylabel('LAeq, Night (dBA)', fontsize=17)
min_value = funza.min()
...
ax_skbo.set_title('Niveles nocturnos de ruido aeronáutico - Aeropuerto Internacional El Dorado -  (Diciembre-2022)', fontsize=20, fontweight='bold')
ax_skbo.set_xlabel('Jornadas Nocturnas', fontsize=17)
ax_skbo.set_ylabel('LAeq, Night (dBA)', fontsize=17)
min_value = skbo.min()
...
ax_suba.set_title('Niveles nocturnos de ruido aeronáutico - Localidad de Suba -  (Diciembre-2022)',fontsize=20, fontweight='bold')
ax_suba.set_xlabel('Jornadas Nocturnas', fontsize=17)
ax_suba.set_ylabel('LAeq, Night (dBA)', fontsize=17)
min_value = suba.min()
...
plt.tight_layout()
plt.show()
plt.savefig('SUBA.png')

"""ENGATIVA DIURNO CONO DE APROXIMACION"""

eng_ca_ln=pd.concat([F1,F2,F10,F18,F33],axis=1)
# Configuración de estilo
sns.set_style("whitegrid")
sns.set_palette("colorblind")
#Borrar datos NaN
eng_ca_ln = eng_ca_ln.dropna()
...
for i in range(len(eng_ca_ln.columns)):
    sns.histplot(data=eng_ca_ln, x=eng_ca_ln.columns[i], color=kernel_color[i], multiple='dodge',
                 bins=int((max(eng_ca_ln.iloc[:, i]) - min(eng_ca_ln.iloc[:, i]))/ bin_width), alpha=alpha,
                 kde=False, label=eng_ca_ln.columns[i], linewidth=2, ax=ax, stat='density')
    kde = sns.kdeplot(data=eng_ca_ln, x=eng_ca_ln.columns[i], bw_method='silverman', color=kernel_color[i], multiple='stack',
                      linestyle=kernel_style[i], linewidth=kernel_width[i], fill=True, alpha=0.5, label=eng_ca_ln.columns[i],
                      ax=ax)
...
plt.legend(title="Estaciones del SVCA", loc="upper right", labels=eng_ca_ln.columns, fontsize=12, frameon=True,
           facecolor='white', framealpha=0.8, markerscale=1)
plt.xlabel('LAeq, Night (dBA)', fontsize=17)
plt.ylabel('Frecuencia (Normalizada)', fontsize=17)
plt.title('HISTOGRAMA DE NIVELES NOCTURNOS DE RUIDO AERONÁUTICO CONO DE APROXIMACIÓN - ENGATIVÁ', fontsize=19, fontweight='bold')
plt.grid(True)
...
min_value = eng_ca_ln.min()
max_value = eng_ca_ln.max()
min_value_eng_ca_ln = min_value.min()
max_value_eng_ca_ln = max_value.max()
plt.xlim(min_value_eng_ca_ln-2,max_value_eng_ca_ln+2)

# Mostrar el gráfico
# sns.despine()
plt.tight_layout()
plt.savefig('ENG_CA_LN.png')
plt.show()

"""ENGATIVA DIURNO COSTADO DE PISTA"""

"""eng_cp_ln=pd.concat([F28],axis=1)
# Configuración de estilo
sns.set_style("whitegrid")
sns.set_palette("colorblind")
#Borrar datos NaN
eng_cp_ln = eng_cp_ln.dropna()
# Configurar los parámetros del histograma
bin_width = 1  # Ancho de los bins
alpha = 0.8  # Opacidad del histograma
density = True  # Normalizar el histograma
...
# Crear el histograma y las curvas de kernel
fig, ax = plt.subplots(figsize=(12, 10))
for i in range(len(eng_cp_ln.columns)):
    sns.histplot(data=eng_cp_ln, x=eng_cp_ln.columns[i], color=kernel_color[i], multiple='dodge',
                 bins=int((max(eng_cp_ln.iloc[:, i]) - min(eng_cp_ln.iloc[:, i]))/ bin_width), alpha=alpha, kde=False,
    ...
    kde = sns.kdeplot(data=eng_cp_ld, x=eng_cp_ld.columns[i], bw_method='silverman', color=kernel_color[i], multiple='stack',
                      linestyle=kernel_style[i], linewidth=kernel_width[i], fill=True, alpha=0.5, label=eng_cp_ln.columns[i],
                      ax=ax)

# Configurar las leyendas y etiquetas del gráfico
plt.legend(title="Estaciones del SVCA", loc="upper right", labels=eng_cp_ln.columns, fontsize=12, frameon=True,
           facecolor='white', framealpha=0.8, markerscale=1)
plt.xlabel('LAeq, Night (dBA)', fontsize=17)
plt.ylabel('Frecuencia (Normalizada)', fontsize=17)
plt.title('HISTOGRAMA DE NIVELES NOCTURNOS DE RUIDO AERONÁUTICO COSTADO DE PISTA - ENGATIVÁ', fontsize=19, fontweight='bold')
plt.grid(True)
plt.tick_params(axis='both', which='major', labelsize=12)
plt.tick_params(axis='both', which='minor', labelsize=10)
...
# Configurar tamaño de ejes
min_value = eng_cp_ln.min()
max_value = eng_cp_ln.max()
min_value_eng_cp_ln = min_value.min()
max_value_eng_cp_ln = max_value.max()
plt.xlim(min_value_eng_cp_ln-2,max_value_eng_cp_ln+2)

# Mostrar el gráfico
# sns.despine()
plt.tight_layout()
plt.savefig('ENG_CP_LN.png')
plt.show()"""

"""FONTIBON DIURNO CONO DE APROXIMACION"""

fon_ca_ln=pd.concat([F3,F21,F23,F25,F30],axis=1)
# Configuración de estilo
sns.set_style("whitegrid")
sns.set_palette("colorblind")
# Borrar datos NaN
fon_ca_ln = fon_ca_ln.dropna()
# Configurar los parámetros del histograma
bin_width = 1  # Ancho de los bins
alpha = 0.8  # Opacidad del histograma
density = True  # Normalizar el histograma
...
# Crear el histograma y las curvas de kernel
fig, ax = plt.subplots(figsize=(12, 10))
for i in range(len(fon_ca_ln.columns)):
    sns.histplot(data=fon_ca_ln, x=fon_ca_ln.columns[i], color=kernel_color[i], multiple='dodge',
                 bins=int((max(fon_ca_ln.iloc[:, i]) - min(fon_ca_ln.iloc[:, i])) / bin_width), alpha=alpha, kde=False,
                 label=fon_ca_ln.columns[i], linewidth=2, ax=ax, stat='density')
    kde = sns.kdeplot(data=fon_ca_ln, x=fon_ca_ln.columns[i], bw_method='silverman', color=kernel_color[i], multiple='stack',
                      linestyle=kernel_style[i], linewidth=kernel_width[i], fill=True, alpha=0.5, label=fon_ca_ln.columns[i],
                      ax=ax)

# Configurar las leyendas y etiquetas del gráfico
plt.legend(title="Estaciones del SVCA", loc="upper right", labels=fon_ca_ln.columns, fontsize=12, frameon=True,
           facecolor='white', framealpha=0.8, markerscale=1)
plt.xlabel('LAeq, Night (dBA)', fontsize=17)
plt.ylabel('Frecuencia (Normalizada)', fontsize=17)
plt.title('HISTOGRAMA DE NIVELES NOCTURNOS DE RUIDO AERONÁUTICO CONO DE APROXIMACIÓN - FONTIBÓN', fontsize=19,
          fontweight='bold')
plt.grid(True)
plt.tick_params(axis='both', which='major', labelsize=12)
plt.tick_params(axis='both', which='minor', labelsize=10)
...
# Configurar tamaño de ejes
min_value = fon_ca_ln.min()
max_value = fon_ca_ln.max()
min_value_fon_ca_ln = min_value.min()
max_value_fon_ca_ln = max_value.max()
plt.xlim(min_value_fon_ca_ln - 2, max_value_fon_ca_ln + 2)

# Mostrar el gráfico
# sns.despine()
plt.tight_layout()
plt.savefig('FON_CA_LN.png')
plt.show()

"""FONTIBON DIURNO COSTADO DE PISTA"""

fon_cp_ln=pd.concat([F7,F27],axis=1)
# Configuración de estilo
sns.set_style("whitegrid")
sns.set_palette("colorblind")
# Borrar datos NaN
fon_cp_ln = fon_cp_ln.dropna()
# Configurar los parámetros del histograma
bin_width = 1  # Ancho de los bins
alpha = 0.8  # Opacidad del histograma
density = True  # Normalizar el histograma
...
# Crear el histograma y las curvas de kernel
fig, ax = plt.subplots(figsize=(12, 10))
for i in range(len(fon_cp_ln.columns)):
    sns.histplot(data=fon_cp_ln, x=fon_cp_ln.columns[i], color=kernel_color[i], multiple='dodge',
                 bins=int((max(fon_cp_ln.iloc[:, i]) - min(fon_cp_ln.iloc[:, i])) / bin_width), alpha=alpha, kde=False,
                 label=fon_cp_ln.columns[i], linewidth=2, ax=ax, stat='density')
    kde = sns.kdeplot(data=fon_cp_ln, x=fon_cp_ln.columns[i], bw_method='silverman', color=kernel_color[i], multiple='stack',
                      linestyle=kernel_style[i], linewidth=kernel_width[i], fill=True, alpha=0.5, label=fon_cp_ln.columns[i],
                      ax=ax)

# Configurar las leyendas y etiquetas del gráfico
plt.legend(title="Estaciones del SVCA", loc="upper right", labels=fon_cp_ln.columns, fontsize=12, frameon=True,
           facecolor='white', framealpha=0.8, markerscale=1)
plt.xlabel('LAeq, Night (dBA)', fontsize=17)
plt.ylabel('Frecuencia (Normalizada)', fontsize=17)
plt.title('HISTOGRAMA DE NIVELES NOCTURNOS DE RUIDO AERONÁUTICO COSTADO DE PISTA - FONTIBÓN', fontsize=19,
          fontweight='bold')
plt.grid(True)
plt.tick_params(axis='both', which='major', labelsize=12)
plt.tick_params(axis='both', which='minor', labelsize=10)
...
# Configurar tamaño de ejes
min_value = fon_cp_ln.min()
max_value = fon_cp_ln.max()
min_value_fon_cp_ln = min_value.min()
max_value_fon_cp_ln = max_value.max()
plt.xlim(min_value_fon_cp_ln - 2, max_value_fon_cp_ln + 2)

# Mostrar el gráfico
# sns.despine()
plt.tight_layout()
plt.savefig('FON_CP_LN.png')
plt.show()

"""FUNZA DIURNO AREA RURAL"""

fun_rural_ln=pd.concat([F5,F11,F17],axis=1)
# Configuración de estilo
sns.set_style("whitegrid")
sns.set_palette("colorblind")
# Borrar datos NaN
fun_rural_ln = fun_rural_ln.dropna()
# Configurar los parámetros del histograma
bin_width = 1  # Ancho de los bins
alpha = 0.8  # Opacidad del histograma
density = True  # Normalizar el histograma
...
# Crear el histograma y las curvas de kernel
fig, ax = plt.subplots(figsize=(12, 10))
for i in range(len(fun_rural_ln.columns)):
    sns.histplot(data=fun_rural_ln, x=fun_rural_ln.columns[i], color=kernel_color[i], multiple='dodge',
                 bins=int((max(fun_rural_ln.iloc[:, i]) - min(fun_rural_ln.iloc[:, i])) / bin_width), alpha=alpha, kde=False,
                 label=fun_rural_ln.columns[i], linewidth=2, ax=ax, stat='density')
    kde = sns.kdeplot(data=fun_rural_ln, x=fun_rural_ln.columns[i], bw_method='silverman', color=kernel_color[i], multiple='stack',
                      linestyle=kernel_style[i], linewidth=kernel_width[i], fill=True, alpha=0.5, label=fun_rural_ln.columns[i],
                      ax=ax)

# Configurar las leyendas y etiquetas del gráfico
plt.legend(title="Estaciones del SVCA", loc="upper right", labels=fun_rural_ln.columns, fontsize=12, frameon=True,
           facecolor='white', framealpha=0.8, markerscale=1)
plt.xlabel('LAeq, Night (dBA)', fontsize=17)
plt.ylabel('Frecuencia (Normalizada)', fontsize=17)
plt.title('HISTOGRAMA DE NIVELES NOCTURNOS DE RUIDO AERONÁUTICO ÁREA RURAL - FUNZA', fontsize=19, fontweight='bold')
plt.grid(True)
plt.tick_params(axis='both', which='major', labelsize=12)
plt.tick_params(axis='both', which='minor', labelsize=10)
...
# Configurar tamaño de ejes
min_value = fun_rural_ln.min()
max_value = fun_rural_ln.max()
min_value_fun_rural_ln = min_value.min()
max_value_fun_rural_ln = max_value.max()
plt.xlim(min_value_fun_rural_ln - 2, max_value_fun_rural_ln + 2)

# Mostrar el gráfico
# sns.despine()
plt.tight_layout()
plt.savefig('FUN_RURAL_LN.png')
plt.show()


"""FUNZA DIURNO AREA URBANA"""

fun_urb_ln=pd.concat([F24,F29],axis=1)
# Configuración de estilo
sns.set_style("whitegrid")
sns.set_palette("colorblind")
# Borrar datos NaN
fun_urb_ln = fun_urb_ln.dropna()
# Configurar los parámetros del histograma
bin_width = 1  # Ancho de los bins
alpha = 0.8  # Opacidad del histograma
density = True  # Normalizar el histograma
...
# Crear el histograma y las curvas de kernel
fig, ax = plt.subplots(figsize=(12, 10))
for i in range(len(fun_urb_ln.columns)):
    sns.histplot(data=fun_urb_ln, x=fun_urb_ln.columns[i], color=kernel_color[i], multiple='dodge',
                 bins=int((max(fun_urb_ln.iloc[:, i]) - min(fun_urb_ln.iloc[:, i])) / bin_width), alpha=alpha, kde=False,
                 label=fun_urb_ln.columns[i], linewidth=2, ax=ax, stat='density')
    kde = sns.kdeplot(data=fun_urb_ln, x=fun_urb_ln.columns[i], bw_method='silverman', color=kernel_color[i], multiple='stack',
                      linestyle=kernel_style[i], linewidth=kernel_width[i], fill=True, alpha=0.5, label=fun_urb_ln.columns[i],
                      ax=ax)

# Configurar las leyendas y etiquetas del gráfico
plt.legend(title="Estaciones del SVCA", loc="upper right", labels=fun_urb_ln.columns, fontsize=12, frameon=True,
           facecolor='white', framealpha=0.8, markerscale=1)
plt.xlabel('LAeq, Night (dBA)', fontsize=17)
plt.ylabel('Frecuencia (Normalizada)', fontsize=17)
plt.title('HISTOGRAMA DE NIVELES NOCTURNOS DE RUIDO AERONÁUTICO ÁREA URBANA - FUNZA', fontsize=19, fontweight='bold')
plt.grid(True)
plt.tick_params(axis='both', which='major', labelsize=12)
plt.tick_params(axis='both', which='minor', labelsize=10)
...
# Configurar tamaño de ejes
min_value = fun_urb_ln.min()
max_value = fun_urb_ln.max()
min_value_fun_urb_ln = min_value.min()
max_value_fun_urb_ln = max_value.max()
plt.xlim(min_value_fun_urb_ln - 2, max_value_fun_urb_ln + 2)

# Mostrar el gráfico
# sns.despine()
plt.tight_layout()
plt.savefig('FUN_URB_LN.png')
plt.show()

"""SKBO DIURNO AREA URBANA"""

skbo_ln=pd.concat([F13, F15, F19, F20],axis=1)
# Configuración de estilo
sns.set_style("whitegrid")
sns.set_palette("colorblind")
# Borrar datos NaN
skbo_ln = skbo_ln.dropna()
# Configurar los parámetros del histograma
bin_width = 1  # Ancho de los bins
alpha = 0.8  # Opacidad del histograma
density = True  # Normalizar el histograma
...
# Crear el histograma y las curvas de kernel
fig, ax = plt.subplots(figsize=(12, 10))
for i in range(len(skbo_ln.columns)):
    sns.histplot(data=skbo_ln, x=skbo_ln.columns[i], color=kernel_color[i], multiple='dodge',
                 bins=int((max(skbo_ln.iloc[:, i]) - min(skbo_ln.iloc[:, i])) / bin_width), alpha=alpha, kde=False,
                 label=skbo_ln.columns[i], linewidth=2, ax=ax, stat='density')
    kde = sns.kdeplot(data=skbo_ln, x=skbo_ln.columns[i], bw_method='silverman', color=kernel_color[i], multiple='stack',
                      linestyle=kernel_style[i], linewidth=kernel_width[i], fill=True, alpha=0.5, label=skbo_ln.columns[i],
                      ax=ax)

# Configurar las leyendas y etiquetas del gráfico
plt.legend(title="Estaciones del SVCA", loc="upper right", labels=skbo_ln.columns, fontsize=12, frameon=True,
           facecolor='white', framealpha=0.8, markerscale=1)
plt.xlabel('LAeq, Night (dBA)', fontsize=17)
plt.ylabel('Frecuencia (Normalizada)', fontsize=17)
plt.title('HISTOGRAMA DE NIVELES NOCTURNOS DE RUIDO AERONÁUTICO - SKBO', fontsize=19, fontweight='bold')
plt.grid(True)
plt.tick_params(axis='both', which='major', labelsize=12)
plt.tick_params(axis='both', which='minor', labelsize=10)
...
# Configurar tamaño de ejes
min_value = skbo_ln.min()
max_value = skbo_ln.max()
min_value_skbo_ln = min_value.min()
max_value_skbo_ln = max_value.max()
plt.xlim(min_value_skbo_ln - 2, max_value_skbo_ln + 2)

# Mostrar el gráfico
# sns.despine()
plt.tight_layout()
plt.savefig('SKBO_LN.png')
plt.show()

"""SUBA"""

suba_ln=pd.concat([F34],axis=1)
# Configuración de estilo
sns.set_style("whitegrid")
sns.set_palette("colorblind")
# Borrar datos NaN
suba_ln = suba_ln.dropna()
# Configurar los parámetros del histograma
bin_width = 1  # Ancho de los bins
alpha = 0.8  # Opacidad del histograma
density = True  # Normalizar el histograma
...
# Crear el histograma y las curvas de kernel
fig, ax = plt.subplots(figsize=(12, 10))
for i in range(len(suba_ln.columns)):
    sns.histplot(data=suba_ln, x=suba_ln.columns[i], color=kernel_color[i], multiple='dodge',
                 bins=int((max(suba_ln.iloc[:, i]) - min(suba_ln.iloc[:, i])) / bin_width), alpha=alpha, kde=False,
                 label=suba_ln.columns[i], linewidth=2, ax=ax, stat='density')
    kde = sns.kdeplot(data=suba_ln, x=suba_ln.columns[i], bw_method='silverman', color=kernel_color[i], multiple='stack',
                      linestyle=kernel_style[i], linewidth=kernel_width[i], fill=True, alpha=0.5, label=suba_ln.columns[i],
                      ax=ax)

# Configurar las leyendas y etiquetas del gráfico
plt.legend(title="Estaciones del SVCA", loc="upper right", labels=suba_ln.columns, fontsize=12, frameon=True,
           facecolor='white', framealpha=0.8, markerscale=1)
plt.xlabel('LAeq, Night (dBA)', fontsize=17)
plt.ylabel('Frecuencia (Normalizada)', fontsize=17)
plt.title('HISTOGRAMA DE NIVELES NOCTURNOS DE RUIDO AERONÁUTICO - SUBA', fontsize=19, fontweight='bold')
plt.grid(True)
plt.tick_params(axis='both', which='major', labelsize=12)
plt.tick_params(axis='both', which='minor', labelsize=10)
...
# Configurar tamaño de ejes
min_value = suba_ln.min()
max_value = suba_ln.max()
min_value_suba_ln = min_value.min()
max_value_suba_ln = max_value.max()
plt.xlim(min_value_suba_ln - 2, max_value_suba_ln + 2)

# Mostrar el gráfico
# sns.despine()
plt.tight_layout()
plt.savefig('SUBA_LN.png')
plt.show()
...
ax.set_title('NIVELES DE RUIDO EQUIVALENTES POR EVENTOS - DIAGRAMA DE VIOLINES - JORNADA NOCTURNA - ENGATIVÁ',fontsize=20, fontweight='bold')
plt.savefig('ENG_LEQ_LN.png')
plt.show()

"""DIAGRAMA DE VIOLINES - LEQ - FONTIBON"""
# sns.kdeplot(data=df_ln, x="LEVEL_LD", y="LEVEL_LD", hue="NMT", fill=True,) Grafica kernel en dos ejes (Prueba)
fon_leq_events=pd.concat([f3_leq_event,f7_leq_event,f21_leq_event,f23_leq_event,f25_leq_event,f27_leq_event,f30_leq_event],axis=1)
f, ax = plt.subplots()
sns.set_theme(style="whitegrid")
...
ax.set_ylabel('LEQ (dBA)',fontsize=17)
ax.set_title('NIVELES DE RUIDO EQUIVALENTES POR EVENTOS - DIAGRAMA DE VIOLINES - JORNADA NOCTURNA - FONTIBÓN',fontsize=20, fontweight='bold')
plt.savefig('FON_LEQ_LN.png')
plt.show()
...
ax.set_ylabel('LEQ (dBA)',fontsize=17)
ax.set_title('NIVELES DE RUIDO EQUIVALENTES POR EVENTOS - DIAGRAMA DE VIOLINES - JORNADA NOCTURNA - FUNZA',fontsize=20, fontweight='bold')
plt.savefig('FUN_LEQ_LN.png')
plt.show()
...
ax.set_ylabel('LEQ (dBA)',fontsize=17)
ax.set_title('NIVELES DE RUIDO EQUIVALENTES POR EVENTOS - DIAGRAMA DE VIOLINES - JORNADA NOCTURNA - SKBO',fontsize=20, fontweight='bold')
plt.savefig('SKBO_LEQ_LN.png')
plt.show()
...
ax.set_ylabel('LEQ (dBA)',fontsize=17)
ax.set_title('NIVELES DE RUIDO EQUIVALENTES POR EVENTOS - DIAGRAMA DE VIOLINES - JORNADA NOCTURNA - SUBA',fontsize=20, fontweight='bold')
plt.savefig('SUBA_LEQ_LN.png')
plt.show()

"""CREACIÓN DE .XLSX"""

r_aero=pd.ExcelWriter('Aeronautico_LN_Oct.xlsx')#Re escribe el archivo guardado con el mismo nombre, falta unir
df1=pd.DataFrame(df)

df.to_excel(r_aero,'Events')
df['FIS Aircraft type'].isna().sum()#Cuenta la cantidad de datos Nan en una columna esecifica
df_final.to_excel(r_aero,'LN_Hora')
nocturno.to_excel(r_aero,'LN_Dia')
LA.to_excel(r_aero,'Fase II')
l_cinco.to_excel(r_aero,'00-05')
l_siete.to_excel(r_aero,'00-07')
l_cinco_siete.to_excel(r_aero,'05-07')
Count_Events.to_excel(r_aero,'Events_NMT')
freq_table.to_excel(r_aero, 'Freq_Table')
niv_air_type_perday.to_excel(r_aero, 'Nivel_type_night')
niv_air_type_permonth.to_excel(r_aero, 'Nivel_type_month')
niv_air_perday.to_excel(r_aero, 'Nivel_type_night_f')
...
top_niv_air_perday_max.to_excel(r_aero,'Lmax_airtype_night')
